...
import com.cabservice.dao.DriverUseCarsDao;
...
public class DriverController {
   ...
   @RequestMapping("list-drivers")
    public String listAllDrivers(ModelMap modelMap) {
        List<java.sql.Driver> drivers=driverRepository.findAll();
   ...
   @ResponseBody
    public DriverUseCarsDao getCarsUsedByDriver(@RequestParam int driverId) throws Exception {
        Optional<java.sql.Driver> driverOptional=driverRepository.findById(driverId);
        if(driverOptional.isPresent()) {
           Driver driver=(Driver) driverOptional.get();
            ...
            List<Car> carList=carRepository.findByIdIn(carIds)DriverUseCarsDao driverUseCarsDao=new DriverUseCarsDao(driverId,driver.getUsername(),carlist);
    ...
    public String deleteDriver(@RequestParam int id) throws Exception {
        Driver driver=(Driver) driverRepository.findById(id).orElseThrow(() ->
